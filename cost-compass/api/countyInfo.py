...
def get_county_data(county):
    ...
    url = "https://api.census.gov/data/2022/acs/acs5?get=NAME,B25077_001E&for=county:*&in=state:12&key={}".format(census_api_key)
    response = requests.request("GET", url)
    home_val_df = json_to_dataframe(response)
    home_val_df = home_val_df[home_val_df['NAME'] == f"{county}, Florida"]
    
    home_val = home_val_df['B25077_001E'].iloc[0]

    # Median Monthly Housing Cost
    url = "https://api.census.gov/data/2022/acs/acs5?get=NAME,B25105_001E&for=county:*&in=state:12&key={}".format(census_api_key)
    response = requests.request("GET", url)
    housing_cost_df = json_to_dataframe(response)
    housing_cost_df = housing_cost_df[housing_cost_df['NAME'] == f"{county}, Florida"]

    housing_cost = housing_cost_df['B25105_001E'].iloc[0]

    # Median Household Income (in the last 12 months)
    url = "https://api.census.gov/data/2022/acs/acs5?get=NAME,B19013_001E&for=county:*&in=state:12&key={}".format(census_api_key)
    response = requests.request("GET", url)
    household_income_df = json_to_dataframe(response)
    household_income_df = household_income_df[household_income_df['NAME'] == f"{county}, Florida"]
